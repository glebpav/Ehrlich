...
DEFAULT_AMIN_THRESHOLD = 0.5
DEFAULT_GEOM_THRESHOLD = 0.5


def filter_paris_data(
        pairs: List[Tuple[Segment, Segment]],
        extended_filter_function: Callable[[Segment, Segment], (bool, float)],
        top_paris: int
    ...
    for pair_idx, (segment1, segment2) in enumerate(pairs):
        pair_value_list[pair_idx], is_pair_passed_list[pair_idx] = extended_filter_function(segment1, segment2)
    ...
    geometry_top_paris_idxs = np.argpartition(pair_value_list, after_filter_size)
...
def filter_segments_by_descriptors(
        pairs: List[Tuple[Segment, Segment]],
        descr_geom_filter: Callable[[Segment, Segment], (bool, float)] = default_geom_descr_filter,
        descr_gtop: int = 15,
        descr_amin_filter: Callable[[Segment, Segment], (bool, float)] = default_amin_descr_filter,
        descr_atop: int = 10
) -> np.array:

    descriptors_geometry_top = filter_paris_data(pairs, descr_geom_filter, descr_gtop)
    descriptors_amin_top = filter_paris_data(descriptors_geometry_top, descr_amin_filter, descr_atop)
...
def filter_segments_by_alignment(
        pairs: List[Tuple[Segment, Segment]],
        align_segment_filter: Callable[[Segment, Segment], (bool, float)] = default_segment_align_filter,
        align_segment_top: int = 5,
        align_mol_filter: Callable[[Segment, Segment], (bool, float)] = default_molecule_align_filter,
        align_mtop: int = 3,
) -> np.array:
